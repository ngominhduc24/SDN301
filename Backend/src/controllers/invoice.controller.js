  ...
  try {
    ...
    if (invoiceData.details && invoiceData.details.length > 0) {
      for (const detail of invoiceData.details) {
        ...
        if (
          invoiceData.from != null &&
          invoiceData.from != undefined &&
          invoiceData.from != ""
        ) {
          var productFrom = await ShopService.getProductById(
            invoiceData.from,
            detail.productId
          );
          if (!productFrom) {
            throw new Error(
              "Not exists productId" +
                detail.productId +
                "in shop: " +
                invoiceData.from
            );
          ...
          if (productFrom.quantity - detail.quantity < 0) {
            throw new Error(
              "Invalid quantity for productId: " + detail.productId
            );
          }
        }
    ...
    invoiceData.total_price =
      invoiceData.sub_total - discountAmount + invoiceData.shipping_charge;
    ...
    if (req.body.requestId) {
      request = await RequestService.updateRequest(req.body.requestId, {
...
async function updateInfo(req, res, next) {
  try {
    ...
    const discountAmount =
      (updateInvoice.discount / 100) * updateInvoice.sub_total;
    updateInvoice.total_price =
      updateInvoice.sub_total - discountAmount + updateInvoice.shipping_charge;
    ...
    if (
      ...
      if (updateInvoice.details && updateInvoice.details.length > 0) {
        await Promise.all(
          updateInvoice.details.map((detail) =>
            ShopService.updateProductById(
              invoice.from,
              detail.productId,
              -detail.quantity
...
async function updateStatus(req, res, next) {
  try {
    ...
    if (
      ...
      throw new Error(
        `Invoice ${invoice.status} cannot be updated to ${newStatus}`
      );
      ...
      if (invoice.to != null && invoice.to != undefined && invoice.to != "") {
        await Promise.all(
          invoice.details.map(async (detail) => {
            const product = await ShopService.getProById(
              invoice.to,
              detail.productId
            );
            if (product) {
              await ShopService.updateProductById(
                invoice.to,
                detail.productId,
                detail.quantity
              );
            } else {
              await ShopService.createOneProduct(invoice.to, {
    ...
    if (
      ...
      if (
        ...
        for (const detail of invoice.details) {
          const product = await ProductService.getProductById(detail.productId);
          const productFrom = await ShopService.getProductById(
            invoice.from,
            detail.productId
          );
          ...
          if (
            ...
            productFrom.status === "inactive"
          ) {
            throw new Error(
              "Can't change this to pending because productId: " +
                detail.productId +
                " is already active"
            );
          }
...
async function exportInvoiceToPDF(req, res) {
  try {
    ...
    const filePath = pdfFilePath.replace("/s3/", "/object/public/");
    ...
    res
      .status(500)
      .json({ message: "Error exporting invoice", errors: error.message });
  }
}
